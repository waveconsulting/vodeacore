<div class="input-group amount-wrapper">
	...
	<input class="form-control autonumeric" value="{{ $model->getValue($key, $listItem, $language) }}" type="text" {{ $model->isRequired($key) }} {{ $model->isDisabled($key) }} label="{{ $model->label($key) }}"/>
